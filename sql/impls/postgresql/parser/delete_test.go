...
func TestParseDeleteQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:    "DELETE FROM users WHERE email = $1",
			want1:   []int{0},
			wantErr: false,
		},
		{
			...
			want:    "DELETE FROM users WHERE (is_active = 0 AND last_login < $1)",
			want1:   []int{0},
			wantErr: false,
		},
		{
...
func TestParseSoftDeleteByIDQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    "UPDATE users SET deleted = 1 WHERE id = $1",
			wantErr: false,
		},
		{
...
func TestParseSoftDeleteQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:    "UPDATE users SET deleted = 1 WHERE email = $1",
			want1:   []int{0},
			wantErr: false,
		},
		{
			...
			want:    "UPDATE users SET deleted = 1 WHERE (is_active = 0 OR last_login < $1)",
			want1:   []int{0},
			wantErr: false,
		},
		{
