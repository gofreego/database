...
func TestParseUpsertQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    "INSERT INTO users (name, email, password_hash, is_active, created_at, updated_at) VALUES (@p1, @p2, @p3, @p4, @p5, @p6) ON DUPLICATE KEY UPDATE name = VALUES(name), email = VALUES(email), password_hash = VALUES(password_hash), is_active = VALUES(is_active), created_at = VALUES(created_at), updated_at = VALUES(updated_at)",
